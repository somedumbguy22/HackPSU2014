...
if(navigator.geolocation) {
    ...
    navigator.geolocation.getCurrentPosition(function(position) {
        CENTER_POS = new google.maps.LatLng(position.coords.latitude,
...
//    map = new google.maps.Map(document.getElementById('map-canvas'), mapOptions);
    ...
    istPosition = CENTER_POS;
    //directionsDisplay.setMap(map);
    });
    //MAP = map;
}
directionsDisplay.setMap(map);
    console.log(istPosition);
//var mapOptions = {
//zoom: 15,
//center: ISTLatLng
//}
...
function makeMarker(map, marker,lat_long, image, carNumber){
 ...
 google.maps.event.addListener(marker, 'click', function(){
  ...
  else if(YOU_HAVE_REQUESTED_A_TAXI && marker == REQUESTED_TAXI && !markerStateTracker)
  {
    ...
    sendCancelation(currentTaxiNumber, currentNum);
  }
  else if(YOU_HAVE_REQUESTED_A_TAXI && marker == REQUESTED_TAXI && markerStateTracker)
  {
    directionsDisplay.setMap(map);
...
function callback(response, status) {
 if (status != google.maps.DistanceMatrixStatus.OK) {
   ...
   //THE_MINUTES = results[0].duration.text;
...
var sendCancelation = function(currentTaxiNumber, currentNum) {
    $.get("http://ezriderapp.com/sendCancel?", {
        phone_number: currentNum,
        taxi_number: currentTaxiNumber
    }).done(function() {
    }).error(function(xhr, ajaxOptions, thrownError) {
        alert("Track add failed\nError code: " + xhr.status + " " + thrownError);
